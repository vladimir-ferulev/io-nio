...
// обработчик для любых типов HTTP запросов
public class CustomHttpServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) {        // вызывается отдельно для самого HTTP запроса и для тела HTTP запроса
        if (msg instanceof HttpRequest) {
            ...
            responseData.append(RequestUtils.formatParams(request));            // в ответ добавляются параметры из URL
        }

        responseData.append(RequestUtils.evaluateDecoderResult(request));

        // вызывается для всех типов HTTP, но данные будут добавлены только если HTTP запрос с телом, например POST запрос
